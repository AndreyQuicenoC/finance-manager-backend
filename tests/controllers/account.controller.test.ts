...
describe('AccountController', () => {
  ...
  describe('getAccountsByUser', () => {
    it('should return accounts filtered by userId param', async () => {
  ...
  describe('updateAccount', () => {
    ...
    it('should use existing name when name is not provided in update', async () => {
      const req = {
        params: { id: '10' },
        body: { money: 500 },
      } as unknown as Request;
      ...
      const updated = { ...existing, money: 500 };
      prismaMock.account.findUnique.mockResolvedValueOnce(existing as any);
      prismaMock.account.update.mockResolvedValueOnce(updated as any);
      ...
      expect(prismaMock.account.update).toHaveBeenCalledWith({
        where: { id: 10 },
        data: {
          name: 'Original', // Should use existing name when not provided
          money: 500,
          categoryId: 3,
        },
      });
